> **Source:** [jgraph/drawio-mcp `shared/xml-reference.md`](https://github.com/jgraph/drawio-mcp/blob/main/shared/xml-reference.md) (Apache-2.0). Vendored in this skill for offline authoring and for alignment with `drawio_svg.py` reference checks. Re-sync from upstream when updating.
>
> **This skill:** For **explicit edge `strokeColor` / arrow** defaults and **UML sequence lifeline** modeling (so `.drawio.svg` stays readable in git), see **`SKILL.md`** in this folder — sections *Visible connectors* and *UML sequence diagrams*.
