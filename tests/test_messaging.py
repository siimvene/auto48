...
async def test_start_conversation(msg_client, seeded_ids):
    ...
    resp = await msg_client.post(
        "/v1/conversations",
        json={"listing_id": listing_id, "buyer_id": buyer_id},
    )
...
async def test_post_and_list_messages_ordered(msg_client, seeded_ids):
    ...
    msg1 = await msg_client.post(
        ...
        json={"sender_id": buyer_id, "body": "Hello, is it still available?"},
    )
    msg2 = await msg_client.post(
        f"/v1/conversations/{conv_id}/messages",
        json={"sender_id": buyer_id, "body": "Can you do 24 000?"},
    )
...
async def test_list_conversations_for_user(msg_client, seeded_ids):
    ...
    # Buyer can see their conversation.
    buyer_resp = await msg_client.get(
        "/v1/conversations", params={"user_id": buyer_id}
    )
    assert buyer_resp.status_code == 200
    assert any(c["buyer_id"] == buyer_id for c in buyer_resp.json())

    # Seller can also see it.
    seller_resp = await msg_client.get(
        "/v1/conversations", params={"user_id": seller_user_id}
    )
    assert seller_resp.status_code == 200
    assert any(c["seller_id"] == seller_user_id for c in seller_resp.json())


async def test_post_message_missing_conversation(msg_client, seeded_ids):  # noqa: ARG001
    resp = await msg_client.post(
        "/v1/conversations/999999/messages",
        json={"sender_id": 1, "body": "ghost message"},
    )
    assert resp.status_code == 404


async def test_start_conversation_missing_listing(msg_client, seeded_ids):  # noqa: ARG001
    resp = await msg_client.post(
        "/v1/conversations",
        json={"listing_id": 999999, "buyer_id": 1},
    )
    assert resp.status_code == 404
